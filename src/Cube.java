...
public class Cube {
	...
	public void reset() {
		...
		for (int i = 0; i <= 1; i++) {
			for (int j = 0; j <= 1; j++) {
				for (int k = 0; k <= 1; k++) {
					pts[i][j][k] = new CubePoint(i * length - length / 2, j * length - length / 2,
							k * length - length / 2);
				}
			}
		}
	...
	public void updateFaces() {
		...
		for (int i = 0; i <= 1; i++) {
			for (int j = 0; j <= 1; j++) {
				for (int k = 0; k <= 1; k++) {
					if (i + 1 < 2 && j + 1 < 2) {
						faces.add(Face.makeFace(new CubePoint[] { pts[i][j][k], pts[i + 1][j][k], pts[i + 1][j + 1][k],
								pts[i][j + 1][k] }));
					}

					if (j + 1 < 2 && k + 1 < 2) {
						faces.add(Face.makeFace(new CubePoint[] { pts[i][j][k], pts[i][j + 1][k], pts[i][j + 1][k + 1],
								pts[i][j][k + 1] }));
					}

					if (i + 1 < 2 && k + 1 < 2) {
						faces.add(Face.makeFace(new CubePoint[] { pts[i][j][k], pts[i + 1][j][k], pts[i + 1][j][k + 1],
								pts[i][j][k + 1] }));
					}
				}
			}
